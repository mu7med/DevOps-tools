...
Environment Variables:
    ...
    GIT_OWNER - default: mohammed-abdullah - used only when omitting the owner/ prefix of owner/repo
"
...
git_owner="${GIT_OWNER:-mohammed-abdullah}"
...
if [ $# -gt 0 ]; then
    ...
    if [ -n "$repolist" ]; then
        ...
        repolist="$(curl -sSL https://raw.githubusercontent.com/mohammed-abdullah/bash-tools/master/setup/repos.txt | sed 's/#.*//')"
    fi
fi
